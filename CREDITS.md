...
| Neon Street floor variants A–D | `public/textures/floors/neon_street_tile_{a,b,c,d}.webp` | Provided by the project owner (original art) | Owned / original | 512×512 flat top-down tiles (uploaded as .png, actually webp). Composed into an N×N atlas (deterministic per-cell pick) when `RENDER.floorTileVariants` is on, for the Neon Street stage. ⚠ Each tile has a baked-in "A/B/C/D" corner label (annotation) — needs clean versions before that stage ships. |
